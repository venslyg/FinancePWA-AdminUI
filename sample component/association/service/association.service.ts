...
type RestOf<T extends IAssociation | NewAssociation | PartialUpdateAssociation> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class AssociationService {
  ...
  protected resourceUrl = `/services/operationsmodule/api/associations`;

  create(payload: NewAssociation): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.post<RestAssociation>(this.resourceUrl, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  update(payload: IAssociation): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.put<RestAssociation>(`${this.resourceUrl}/${payload.id}`, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<RestAssociation>(`${this.resourceUrl}/${id}`, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = this.createRequestOption(req);
    return this.http.get<RestAssociation[]>(this.resourceUrl, { params: options, observe: 'response' }).pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IAssociation | NewAssociation | PartialUpdateAssociation>(entity: T): RestOf<T> {
  ...
  protected convertDateFromServer(restEntity: RestAssociation): IAssociation {
