...
import { BudgetPlanFormComponent } from '../form/budget-plan-form.component';
...
export class BudgetPlanListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<BudgetPlanListComponent>, { optional: true });
  ...
  selectedBudgetPlan: IBudgetPlan | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    
    {
      ...
      rawFieldType: 'BudgetAlertStatus',
      enumOptionsKey: 'budgetAlertStatusOptions'
    },
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'accountCode', 'budgetPlanCode', 'departmentName', 'year', 'allocatedAmount', 'spentAmount', 'remainingAmount', 'usedPercentage', 'alertStatus', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IBudgetPlan>();
  ...
  loadData(): void {
    ...
    this.budgetPlanService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.budgetPlanService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedBudgetPlan = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedBudgetPlan = null;
      this.formDrawer.open();
    }
  }
