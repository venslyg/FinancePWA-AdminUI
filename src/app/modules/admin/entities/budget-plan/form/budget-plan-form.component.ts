...
import { BudgetPlanFormGroup, BudgetPlanFormService } from '../update/budget-plan-form.service';
...
type BudgetPlanFormDialogData = {
  entity?: IBudgetPlan;
  defaults?: Partial<NewBudgetPlan>;
  heading?: string;
};

@Component({
  selector: 'app-budget-plan-form',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class BudgetPlanFormComponent implements OnInit, OnChanges {
  ...
  private readonly formService = inject(BudgetPlanFormService);
  private readonly dialogRef = inject(MatDialogRef<BudgetPlanFormComponent>, { optional: true });
  private readonly dialogData = inject(MAT_DIALOG_DATA, { optional: true }) as BudgetPlanFormDialogData | null;

  @Input() entity: IBudgetPlan | null = null;
  @Input() heading?: string;

  @Output() saved = new EventEmitter<IBudgetPlan>();
  @Output() cancelled = new EventEmitter<void>();

  form: BudgetPlanFormGroup = this.formService.createBudgetPlanFormGroup();
  isSaving = false;
  isInitialized = false;
  errorMessage: string | null = null;

  
  readonly budgetAlertStatusOptions = Object.keys(BudgetAlertStatus);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['entity'] && changes['entity'].currentValue) {
      this.formService.resetForm(this.form, { ...(changes['entity'].currentValue as IBudgetPlan) });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.isSaving = true;
    const payload = this.formService.getBudgetPlan(this.form);
    const isUpdate = payload.id !== null;
    const request$ = isUpdate
      ? this.budgetPlanService.update(payload as IBudgetPlan)
      : this.budgetPlanService.create(payload as NewBudgetPlan);
  ...
  private initializeFormFromInputs(): void {
    ...
    if (entity) {
      ...
      this.formService.resetForm(this.form, { id: null, ...defaults } as Partial<NewBudgetPlan>);
    }
  }
