...
import { InventoryItemFormGroup, InventoryItemFormService } from '../update/inventory-item-form.service';
...
type InventoryItemFormDialogData = {
  entity?: IInventoryItem;
  defaults?: Partial<NewInventoryItem>;
  heading?: string;
};
...
export class InventoryItemFormComponent implements OnInit, OnChanges {
  private readonly inventoryItemService = inject(InventoryItemService);
  private readonly formService = inject(InventoryItemFormService);
  private readonly dialogRef = inject(MatDialogRef<InventoryItemFormComponent>, { optional: true });
  private readonly dialogData = inject(MAT_DIALOG_DATA, { optional: true }) as InventoryItemFormDialogData | null;

  @Input() entity: IInventoryItem | null = null;
  @Input() heading?: string;

  @Output() saved = new EventEmitter<IInventoryItem>();
  @Output() cancelled = new EventEmitter<void>();

  form: InventoryItemFormGroup = this.formService.createInventoryItemFormGroup();
  isSaving = false;
  isInitialized = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['entity'] && changes['entity'].currentValue) {
      this.formService.resetForm(this.form, { ...(changes['entity'].currentValue as IInventoryItem) });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.isSaving = true;
    const payload = this.formService.getInventoryItem(this.form);
    const isUpdate = payload.id !== null;
    const request$ = isUpdate
      ? this.inventoryItemService.update(payload as IInventoryItem)
      : this.inventoryItemService.create(payload as NewInventoryItem);
  ...
  private initializeFormFromInputs(): void {
    ...
    if (entity) {
      ...
      this.formService.resetForm(this.form, { id: null, ...defaults } as Partial<NewInventoryItem>);
    }
  }
