...
type RestOf<T extends IInventoryItem | NewInventoryItem | PartialUpdateInventoryItem> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class InventoryItemService {
  ...
  protected resourceUrl = `//api/inventory-items`;

  create(payload: NewInventoryItem): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.post<RestInventoryItem>(this.resourceUrl, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  update(payload: IInventoryItem): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.put<RestInventoryItem>(`${this.resourceUrl}/${payload.id}`, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<RestInventoryItem>(`${this.resourceUrl}/${id}`, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = this.createRequestOption(req);
    return this.http.get<RestInventoryItem[]>(this.resourceUrl, { params: options, observe: 'response' }).pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IInventoryItem | NewInventoryItem | PartialUpdateInventoryItem>(entity: T): RestOf<T> {
  ...
  protected convertDateFromServer(restEntity: RestInventoryItem): IInventoryItem {
