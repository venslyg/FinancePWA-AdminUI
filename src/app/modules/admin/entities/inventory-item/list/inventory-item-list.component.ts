...
export class InventoryItemListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<InventoryItemListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.inventoryItemService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.inventoryItemService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedInventoryItem = response.body;
