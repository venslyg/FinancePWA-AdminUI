...
import { BankLedgerFormComponent } from '../form/bank-ledger-form.component';
...
@Component({
  ...
  imports: [
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    BankLedgerFormComponent,
  ],
  templateUrl: './bank-ledger-list.component.html',
})
export class BankLedgerListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<BankLedgerListComponent>, { optional: true });
  ...
  selectedBankLedger: IBankLedger | null = null;
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'bankLedgerCode', 'date', 'referenceNo', 'description', 'depositAmount', 'withdrawalAmount', 'runningBalance', 'remark', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IBankLedger>();
  ...
  loadData(): void {
    ...
    this.bankLedgerService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.bankLedgerService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedBankLedger = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedBankLedger = null;
      this.formDrawer.open();
    }
  }
