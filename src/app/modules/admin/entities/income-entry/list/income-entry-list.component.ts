...
import { IncomeEntryFormComponent } from '../form/income-entry-form.component';
...
@Component({
  selector: 'app-income-entry-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './income-entry-list.component.html',
})
export class IncomeEntryListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<IncomeEntryListComponent>, { optional: true });
  ...
  selectedIncomeEntry: IIncomeEntry | null = null;
  ...
  loadData(): void {
    ...
    this.incomeEntryService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.incomeEntryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedIncomeEntry = response.body;
