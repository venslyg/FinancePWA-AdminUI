...
import { ExpenseEntryFormComponent } from '../form/expense-entry-form.component';
...
export class ExpenseEntryListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<ExpenseEntryListComponent>, { optional: true });
  ...
  selectedExpenseEntry: IExpenseEntry | null = null;
  ...
  loadData(): void {
    ...
    this.expenseEntryService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.expenseEntryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedExpenseEntry = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedExpenseEntry = null;
      this.formDrawer.open();
    }
  }
