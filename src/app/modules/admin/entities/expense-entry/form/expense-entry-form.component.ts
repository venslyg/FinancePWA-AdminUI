...
import { ExpenseEntryFormGroup, ExpenseEntryFormService } from '../update/expense-entry-form.service';
...
type ExpenseEntryFormDialogData = {
  entity?: IExpenseEntry;
  defaults?: Partial<NewExpenseEntry>;
  heading?: string;
};
...
export class ExpenseEntryFormComponent implements OnInit, OnChanges {
  private readonly expenseEntryService = inject(ExpenseEntryService);
  private readonly formService = inject(ExpenseEntryFormService);
  private readonly dialogRef = inject(MatDialogRef<ExpenseEntryFormComponent>, { optional: true });
  private readonly dialogData = inject(MAT_DIALOG_DATA, { optional: true }) as ExpenseEntryFormDialogData | null;

  @Input() entity: IExpenseEntry | null = null;
  @Input() heading?: string;

  @Output() saved = new EventEmitter<IExpenseEntry>();
  @Output() cancelled = new EventEmitter<void>();

  form: ExpenseEntryFormGroup = this.formService.createExpenseEntryFormGroup();
  isSaving = false;
  isInitialized = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['entity'] && changes['entity'].currentValue) {
      this.formService.resetForm(this.form, { ...(changes['entity'].currentValue as IExpenseEntry) });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.isSaving = true;
    const payload = this.formService.getExpenseEntry(this.form);
    const isUpdate = payload.id !== null;
    const request$ = isUpdate
      ? this.expenseEntryService.update(payload as IExpenseEntry)
      : this.expenseEntryService.create(payload as NewExpenseEntry);
  ...
  private initializeFormFromInputs(): void {
    ...
    if (entity) {
      ...
      this.formService.resetForm(this.form, { id: null, ...defaults } as Partial<NewExpenseEntry>);
    }
  }
