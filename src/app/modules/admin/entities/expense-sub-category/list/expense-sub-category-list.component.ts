...
import { ExpenseSubCategoryFormComponent } from '../form/expense-sub-category-form.component';
...
@Component({
  selector: 'app-expense-sub-category-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './expense-sub-category-list.component.html',
})
export class ExpenseSubCategoryListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<ExpenseSubCategoryListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.expenseSubCategoryService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.expenseSubCategoryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedExpenseSubCategory = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedExpenseSubCategory = null;
      this.formDrawer.open();
    }
  }
