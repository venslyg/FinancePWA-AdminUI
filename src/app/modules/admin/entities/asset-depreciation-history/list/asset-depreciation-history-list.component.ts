...
import { AssetDepreciationHistoryFormComponent } from '../form/asset-depreciation-history-form.component';
...
@Component({
  selector: 'app-asset-depreciation-history-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    AssetDepreciationHistoryFormComponent,
  ],
  templateUrl: './asset-depreciation-history-list.component.html',
})
export class AssetDepreciationHistoryListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<AssetDepreciationHistoryListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.assetDepreciationHistoryService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.assetDepreciationHistoryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAssetDepreciationHistory = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedAssetDepreciationHistory = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete AssetDepreciationHistory',
