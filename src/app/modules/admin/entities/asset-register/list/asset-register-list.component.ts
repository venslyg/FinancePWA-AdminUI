...
import { AssetRegisterFormComponent } from '../form/asset-register-form.component';
...
@Component({
  selector: 'app-asset-register-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './asset-register-list.component.html',
})
export class AssetRegisterListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<AssetRegisterListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.assetRegisterService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.assetRegisterService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAssetRegister = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedAssetRegister = null;
      this.formDrawer.open();
    }
  }
