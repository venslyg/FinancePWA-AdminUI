...
export class ExpenseCategoryListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<ExpenseCategoryListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'categoryCode', 'categoryName', 'description', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.expenseCategoryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedExpenseCategory = response.body;
