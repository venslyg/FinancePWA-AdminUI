...
export class AssetCategoryListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<AssetCategoryListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'assetCategoryCode', 'assetCategoryName', 'description', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.assetCategoryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAssetCategory = response.body;
