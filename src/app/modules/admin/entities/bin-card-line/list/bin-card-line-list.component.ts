...
import { BinCardLineFormComponent } from '../form/bin-card-line-form.component';
...
@Component({
  selector: 'app-bin-card-line-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    BinCardLineFormComponent,
  ],
  templateUrl: './bin-card-line-list.component.html',
})
export class BinCardLineListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<BinCardLineListComponent>, { optional: true });
  ...
  selectedBinCardLine: IBinCardLine | null = null;
  ...
  displayedColumns: string[] = ['id', 'inventoryItemCode', 'date', 'referenceNo', 'description', 'quantityIn', 'quantityOut', 'runningBalance', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IBinCardLine>();
  ...
  loadData(): void {
    ...
    this.binCardLineService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.binCardLineService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedBinCardLine = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedBinCardLine = null;
      this.formDrawer.open();
    }
  }
