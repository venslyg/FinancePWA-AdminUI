...
type RestOf<T extends IBinCardLine | NewBinCardLine | PartialUpdateBinCardLine> = Omit<T, 'date' | 'createdDate' | 'lastModifiedDate'> & {
...
export class BinCardLineService {
  ...
  protected resourceUrl = `/api/bin-card-lines`;

  create(payload: NewBinCardLine): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.post<RestBinCardLine>(this.resourceUrl, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  update(payload: IBinCardLine): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.put<RestBinCardLine>(`${this.resourceUrl}/${payload.id}`, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<RestBinCardLine>(`${this.resourceUrl}/${id}`, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = this.createRequestOption(req);
    return this.http.get<RestBinCardLine[]>(this.resourceUrl, { params: options, observe: 'response' }).pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IBinCardLine | NewBinCardLine | PartialUpdateBinCardLine>(entity: T): RestOf<T> {
  ...
  protected convertDateFromServer(restEntity: RestBinCardLine): IBinCardLine {
