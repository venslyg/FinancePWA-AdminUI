...
import { SalaryPayoutFormComponent } from '../form/salary-payout-form.component';
...
@Component({
  selector: 'app-salary-payout-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    SalaryPayoutFormComponent,
  ],
  templateUrl: './salary-payout-list.component.html',
})
export class SalaryPayoutListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<SalaryPayoutListComponent>, { optional: true });
  ...
  selectedSalaryPayout: ISalaryPayout | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    {
      key: 'salaryPayoutCode',
      label: 'SalaryPayoutCode',
      valueType: 'string' as FilterValueType,
      operators: FILTER_OPERATOR_LIBRARY['string'],
      rawFieldType: 'String'
    },
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'salaryPayoutCode', 'staffCode', 'payPeriod', 'baseSalary', 'allowances', 'deductions', 'netPay', 'payoutDate', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<ISalaryPayout>();
  ...
  loadData(): void {
    ...
    this.salaryPayoutService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.salaryPayoutService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedSalaryPayout = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedSalaryPayout = null;
      this.formDrawer.open();
    }
  }
