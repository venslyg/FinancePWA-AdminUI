...
import { PettyCashLedgerFormComponent } from '../form/petty-cash-ledger-form.component';
...
@Component({
  selector: 'app-petty-cash-ledger-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    PettyCashLedgerFormComponent,
  ],
  templateUrl: './petty-cash-ledger-list.component.html',
})
export class PettyCashLedgerListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<PettyCashLedgerListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.pettyCashLedgerService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.pettyCashLedgerService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedPettyCashLedger = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedPettyCashLedger = null;
      this.formDrawer.open();
    }
  }
