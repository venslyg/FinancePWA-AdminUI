...
export class BranchListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<BranchListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'branchName', 'location', 'phoneNumber', 'isActive', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IBranch>();
  ...
  loadData(): void {
    ...
    this.branchService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.branchService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedBranch = response.body;
