...
import { BranchFormGroup, BranchFormService } from '../update/branch-form.service';
...
export class BranchFormComponent implements OnInit, OnChanges {
  ...
  private readonly formService = inject(BranchFormService);
  private readonly dialogRef = inject(MatDialogRef<BranchFormComponent>, { optional: true });
  private readonly dialogData = inject(MAT_DIALOG_DATA, { optional: true }) as BranchFormDialogData | null;

  @Input() entity: IBranch | null = null;
  @Input() heading?: string;

  @Output() saved = new EventEmitter<IBranch>();
  @Output() cancelled = new EventEmitter<void>();

  form: BranchFormGroup = this.formService.createBranchFormGroup();
  isSaving = false;
  isInitialized = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['entity'] && changes['entity'].currentValue) {
      this.formService.resetForm(this.form, { ...(changes['entity'].currentValue as IBranch) });
    }
  }
  ...
  onSubmit(): void {
    ...
    this.isSaving = true;
    const payload = this.formService.getBranch(this.form);
    const isUpdate = payload.id !== null;
    const request$ = isUpdate
      ? this.branchService.update(payload as IBranch)
      : this.branchService.create(payload as NewBranch);
  ...
  private initializeFormFromInputs(): void {
    ...
    if (entity) {
      ...
      this.formService.resetForm(this.form, { id: null, ...defaults } as Partial<NewBranch>);
    }
  }
