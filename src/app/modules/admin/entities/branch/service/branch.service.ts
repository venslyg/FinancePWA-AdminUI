...
export class BranchService {
  ...
  protected resourceUrl = `/api/branches`;
  ...
  update(payload: IBranch): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.put<RestBranch>(`${this.resourceUrl}/${payload.id}`, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }
