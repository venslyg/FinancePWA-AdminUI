...
import { LiabilityLogFormComponent } from '../form/liability-log-form.component';
...
@Component({
  selector: 'app-liability-log-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    LiabilityLogFormComponent,
  ],
  templateUrl: './liability-log-list.component.html',
})
export class LiabilityLogListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<LiabilityLogListComponent>, { optional: true });
  ...
  selectedLiabilityLog: ILiabilityLog | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    
    {
      ...
      rawFieldType: 'LiabilityType',
      enumOptionsKey: 'liabilityTypeOptions'
    },
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'liabilityCode', 'loanFrom', 'description', 'liabilityType', 'totalLoanAmount', 'startDate', 'endDate', 'interestPercentage', 'monthlyPaymentAmount', 'principalPaid', 'balanceToPay', 'status', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<ILiabilityLog>();
  ...
  loadData(): void {
    ...
    this.liabilityLogService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.liabilityLogService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedLiabilityLog = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedLiabilityLog = null;
      this.formDrawer.open();
    }
  }
