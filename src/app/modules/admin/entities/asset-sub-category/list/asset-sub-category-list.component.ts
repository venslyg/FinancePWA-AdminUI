...
import { AssetSubCategoryFormComponent } from '../form/asset-sub-category-form.component';
...
@Component({
  ...
  imports: [
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    AssetSubCategoryFormComponent,
  ],
  templateUrl: './asset-sub-category-list.component.html',
})
export class AssetSubCategoryListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<AssetSubCategoryListComponent>, { optional: true });
  ...
  selectedAssetSubCategory: IAssetSubCategory | null = null;
  ...
  displayedColumns: string[] = ['id', 'assetCategoryCode', 'assetSubCategoryCode', 'assetSubCategoryName', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IAssetSubCategory>();
  ...
  loadData(): void {
    ...
    this.assetSubCategoryService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.assetSubCategoryService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAssetSubCategory = response.body;
