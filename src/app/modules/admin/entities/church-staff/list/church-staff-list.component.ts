...
import { ChurchStaffFormComponent } from '../form/church-staff-form.component';
...
@Component({
  selector: 'app-church-staff-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    ChurchStaffFormComponent,
  ],
  templateUrl: './church-staff-list.component.html',
})
export class ChurchStaffListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<ChurchStaffListComponent>, { optional: true });
  ...
  selectedChurchStaff: IChurchStaff | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    
    {
      ...
      rawFieldType: 'StaffType',
      enumOptionsKey: 'staffTypeOptions'
    },
    ...
    {
      key: 'hourlyRateOrMonthlySalary',
      label: 'HourlyRateOrMonthlySalary',
  ...
  displayedColumns: string[] = ['id', 'staffCode', 'branchCode', 'fullName', 'position', 'staffType', 'contactNumber', 'hourlyRateOrMonthlySalary', 'isActive', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IChurchStaff>();
  ...
  loadData(): void {
    ...
    this.churchStaffService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.churchStaffService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedChurchStaff = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedChurchStaff = null;
      this.formDrawer.open();
    }
  }
