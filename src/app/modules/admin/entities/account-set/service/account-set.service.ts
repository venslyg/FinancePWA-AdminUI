...
type RestOf<T extends IAccountSet | NewAccountSet | PartialUpdateAccountSet> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class AccountSetService {
  ...
  protected resourceUrl = `//api/account-sets`;

  create(payload: NewAccountSet): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.post<RestAccountSet>(this.resourceUrl, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  update(payload: IAccountSet): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(payload);
    return this.http.put<RestAccountSet>(`${this.resourceUrl}/${payload.id}`, copy, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<RestAccountSet>(`${this.resourceUrl}/${id}`, { observe: 'response' }).pipe(map(res => this.convertResponseFromServer(res)));
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = this.createRequestOption(req);
    return this.http.get<RestAccountSet[]>(this.resourceUrl, { params: options, observe: 'response' }).pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IAccountSet | NewAccountSet | PartialUpdateAccountSet>(entity: T): RestOf<T> {
  ...
  protected convertDateFromServer(restEntity: RestAccountSet): IAccountSet {
