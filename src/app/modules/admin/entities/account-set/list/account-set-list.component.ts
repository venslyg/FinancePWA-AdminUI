...
import { AccountSetFormComponent } from '../form/account-set-form.component';
...
@Component({
  ...
  imports: [
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    AccountSetFormComponent,
  ],
  templateUrl: './account-set-list.component.html',
})
export class AccountSetListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<AccountSetListComponent>, { optional: true });
  ...
  selectedAccountSet: IAccountSet | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    
    {
      ...
      rawFieldType: 'AccountType',
      enumOptionsKey: 'accountTypeOptions'
    },
  ...
  displayedColumns: string[] = ['id', 'branchCode', 'accountCode', 'accountName', 'accountType', 'subCategory', 'remark', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IAccountSet>();
  ...
  loadData(): void {
    ...
    this.accountSetService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.accountSetService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAccountSet = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedAccountSet = null;
      this.formDrawer.open();
    }
  }
