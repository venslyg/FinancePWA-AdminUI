...
import { MaintenanceLogFormComponent } from '../form/maintenance-log-form.component';
...
export class MaintenanceLogListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<MaintenanceLogListComponent>, { optional: true });
  ...
  selectedMaintenanceLog: IMaintenanceLog | null = null;
  ...
  filterFields: FilterField[] = [
    ...
    {
      key: 'maintenanceLogCode',
      label: 'MaintenanceLogCode',
    ...
    
    {
      ...
      rawFieldType: 'MaintenanceLogType',
      enumOptionsKey: 'maintenanceLogTypeOptions'
    },
  ...
  displayedColumns: string[] = ['id', 'maintenanceLogCode', 'logDate', 'logType', 'description', 'cost', 'vendor', 'nextServiceDate', 'note', 'createdBy', 'createdDate', 'lastModifiedBy', 'lastModifiedDate',  'actions'];
  dataSource = new MatTableDataSource<IMaintenanceLog>();
  ...
  loadData(): void {
    ...
    this.maintenanceLogService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.maintenanceLogService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedMaintenanceLog = response.body;
